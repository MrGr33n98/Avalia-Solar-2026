...
class HealthControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should get readiness check' do
    ...
    json_response = JSON.parse(response.body)
    assert_includes %w[ready not_ready], json_response['status']
  ...
  test 'readiness check should verify redis connection' do
    ...
    assert_includes %w[ok error not_configured], redis_check['status']
  end
  ...
  test 'readiness check should verify sidekiq status' do
    ...
    assert_includes %w[ok warning error not_configured], sidekiq_check['status']
  end
