...
class PendingChangesControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @user = User.create!(email: "user@example.com", password: "Aa123456", name: "User", role: "company", company: @company, terms_accepted: true, status: :active)
    @token = JWT.encode({ user_id: @user.id }, Rails.application.secret_key_base, "HS256")
    @headers = { "Authorization" => "Bearer #{@token}" }
    @pc = PendingChange.create!(company: @company, user: @user, change_type: "company_info", status: "pending", data: { attributes: { description: "New" } })
  end
