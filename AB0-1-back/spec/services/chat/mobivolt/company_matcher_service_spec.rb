...
RSpec.describe Chat::Mobivolt::CompanyMatcherService, type: :service do
  ...
  let!(:sponsored_installer) do
    comp = create(:company, status: 'active', segment: 'installer', sponsored: true, rating_avg: 5.0, rating_count: 10,
  ...
  describe '.match' do
    ...
    it 'prioriza empresas patrocinadas (sponsored) primeiro' do
      results = described_class.match({ state: 'MT', city: 'Cuiabá' })
    ...
    it 'aplica fallback para top recomendadas quando não há correspondência geográfica ou de filtros' do
      results = described_class.match({ city: 'Cidade Inexistente Sem Cobertura' })
      expect(results.to_a).not_to be_empty
    end

    it 'retorna no máximo 5 resultados' do
      6.times do
        comp = create(:company, status: 'active', segment: 'installer', state: 'SP', city: 'São Paulo')
        comp.categories << category
      end
