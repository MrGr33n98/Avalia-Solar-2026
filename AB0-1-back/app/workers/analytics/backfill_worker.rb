...
module Analytics
  class BackfillWorker
    ...
    def recalculate_day(day)
      start_ts = day.to_time.beginning_of_day
      end_ts   = (day + 1.day).to_time.beginning_of_day
      ...
      result = ActiveRecord::Base.connection.exec_query(sql, 'BackfillRecalc', [
        [nil, day], [nil, start_ts], [nil, end_ts]
      ])
      
      Rails.logger.info("[G4-Backfill] Completed #{day}. Rows updated: #{result.rows.size}")
    end
