ActiveAdmin.register_page "Ranking Preview" do
  ...
  content title: "Simulador de Ranking Meritocrático" do
    ...
    panel "Filtros de Simulação" do
      form action: admin_ranking_preview_path, method: :get do |f|
        div style: "display: flex; gap: 20px; align-items: flex-end; padding: 10px;" do
          ...
          div do
            ...
            input name: "city", value: params[:city], placeholder: "Ex: Florianópolis", style: "padding: 8px; border: 1px solid #ccc; border-radius: 4px;"
          end
        ...
        if ranked_companies.any?
          table_for ranked_companies do
            column "Posição" do |c|
              ...
              if idx <= 3
                span "🏆 #{idx}º", style: "font-weight: bold; color: #{idx == 1 ? '#D4AF37' : (idx == 2 ? '#C0C0C0' : '#CD7F32')};"
              else
                "#{idx}º"
              end
            end
