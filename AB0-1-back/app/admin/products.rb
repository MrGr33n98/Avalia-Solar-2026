      ...
      div class: 'aa-entity-stack' do
        div(class: 'aa-entity-title') { product.name } +
          div(class: 'aa-entity-subtitle') do
            product.short_description.presence || truncate(product.description.to_s, length: 80)
          end
      end
    end
    ...
    column('Preço') do |product|
      div class: 'aa-metric-stack' do
        div(class: 'aa-metric-value') do
          number_to_currency(product.price || 0, unit: 'R$ ', separator: ',', delimiter: '.')
        end +
    ...
    column('Empresa') do |product|
      div class: 'aa-entity-stack' do
        div(class: 'aa-entity-title aa-entity-title--sm') { product.company&.name || '-' } +
          div(class: 'aa-entity-subtitle') do
            product.categories.first(2).map(&:name).join(', ').presence || 'Sem categoria'
          end
      end
    end
        ...
        if product.can_upload_images?
          if product.images.attached?
            div class: 'product-images' do
              ...
              para "Slots restantes: #{product.remaining_image_slots}" if product.remaining_image_slots.positive?
              if product.plan_upgrade_message_for_images && !product.can_upload_more_images?
                para product.plan_upgrade_message_for_images,
    ...
    f.inputs 'Detalhes do Produto' do
      ...
      if f.object.persisted? && f.object.company
        if f.object.can_upload_images?
          ...
          hint_message += "Você já tem #{current_count} imagens" if current_count.positive?
          hint_message += " e pode adicionar mais #{remaining}" if remaining.positive?
          if f.object.plan_upgrade_message_for_images && remaining.zero?
            hint_message += ". #{f.object.plan_upgrade_message_for_images}"
          end
        else
      ...
      build_category_hierarchy = lambda { |parent_id = nil, depth = 0, memo = []|
