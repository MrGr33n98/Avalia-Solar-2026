  ...
  module V1
    class DashboardController < BaseController
      ...
      def stats
        ...
        growth_rate = if last_month_new.positive?
                        ((this_month_new - last_month_new).to_f / last_month_new * 100).round(1)
                      else
                        this_month_new.positive? ? 100.0 : 0.0
                      end
      ...
      def activity
        ...
        ActiveSupport::Notifications.instrument('dashboard.activity.fetch', limit: limit) do |payload|
          ...
          activities = recent_companies.map do |company|
            {
          ...
          # Recent reviews
          recent_reviews = Review.includes(:company).order(created_at: :desc).limit(5).select(:id, :company_id,
                                                                                              :rating, :created_at)
      ...
      def get_revenue_chart_data(_period)
        ...
        (0..(months_to_fetch - 1)).map do |i|
          ...
          saas_val = saas_data.find { |k, _v| k.to_date.beginning_of_month == date.to_date }&.last || 0
          ads_val = (ads_data.find { |k, _v| k.to_date.beginning_of_month == date.to_date }&.last || 0) / 100.0
      ...
      def time_ago_in_words(time)
        ...
        when 86_400..604_799
          "há #{(seconds / 86_400).to_i} dias"
        else
          "há #{(seconds / 604_800).to_i} semanas"
        end
      end
    end
  end
