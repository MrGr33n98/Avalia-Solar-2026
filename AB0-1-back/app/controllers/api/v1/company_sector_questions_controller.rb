module Api
  module V1
    class CompanySectorQuestionsController < BaseController
      ...
      def create
        return plan_required! if requires_paid_plan_for_new_question?
      ...
      def set_company
        requested_id = params[:company_id] || cookies[:active_company_id]
        ...
        @company =
          if current_user&.admin?
            ...
            if requested_id.present?
              scope.find_by(id: requested_id) || current_user&.company
            else
      ...
      def requires_paid_plan_for_new_question?
        @company.requires_paid_plan_for_sector_question?(new_count: @company.company_sector_questions.size + 1)
      end
