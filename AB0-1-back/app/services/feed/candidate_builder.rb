  ...
  class CandidateBuilder
    ...
    def call
      ...
      when 'following'
        ...
        FeedItem.where(
          "(actor_type = 'User' AND actor_id IN (SELECT user_id FROM reviewer_profiles WHERE id IN (?))) OR " \
          "(actor_type = 'Company' AND actor_id IN (?))",
          followed_creators, followed_companies
        )
      when 'creators'
        FeedItem.where(actor_type: 'User')
      when 'companies'
        FeedItem.where(actor_type: 'Company')
      when 'recent', 'for_you'
        FeedItem.public_items
      else
        FeedItem.public_items
      end
    end
