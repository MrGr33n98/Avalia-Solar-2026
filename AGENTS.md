...
_>_ \***\*Público-alvo:\*\*** _agentes de IA que vão ler ou modificar código deste repositório._

_>_ \***\*Idioma do projeto:\*\*** _a documentação, comentários e comunicação técnica são predominantemente em_ \***\*português (Brasil)\*\*\***. Artifacts do repositório devem seguir esse padrão.\*
...
### 12.1 Regra de ouro: evidência antes de alteração

1. **Leia antes de escrever:** consulte este `AGENTS.md`, o `README.md` e, quando relevante, os MADR/ADR em `docs/architecture/`.
2. **Inspecione o estado real:** não trate documentação, relatórios anteriores, summaries de agentes ou arquivos de template como prova do estado atual do código, banco, CI, containers ou MCPs.
3. **Não invente:** não crie métricas, empresas, avaliações, contatos, IDs, rotas, tabelas, endpoints, configurações, resultados de testes ou fatos que não tenham sido verificados.
4. **Mudança mínima:** altere somente o necessário para a tarefa. Não faça refatorações oportunistas.
5. **Preserve trabalho concorrente:** não sobrescreva mudanças locais que não pertencem à tarefa.
6. **Não enfraqueça validações/testes:** corrija a implementação ou a fixture/factory realmente incorreta; nunca mude a expectativa apenas para obter CI verde.
7. **Declare incerteza:** quando algo não puder ser verificado, diga explicitamente que não foi verificado.

### 12.2 Segurança operacional e Git

- Antes de editar, verifique `git status` e os diffs relevantes.
- Não use `git add .`, `git commit -a`, `git reset --hard`, `git restore .`, `git clean -fd`, force-push ou comandos destrutivos sem autorização explícita.
- Não faça `push` automaticamente.
- Commits devem seguir Conventional Commits.
- Nunca exponha ou versione API keys, PATs, Bearer tokens, senhas, cookies, secrets ou conteúdo de `.env`.
- Ao auditar configurações, prefira exibir apenas nomes/chaves e valores comprovadamente não sensíveis.
- Não execute escrita em banco de produção durante diagnóstico, auditoria ou teste.
- Não remova volumes Docker, rode `docker system prune`, derrube stacks não relacionadas ou altere infraestrutura principal para resolver problema local.
- Trate o stack DEV, TEST/CI e produção/main como ambientes distintos. Confirme projeto Compose, arquivo Compose, container e banco antes de qualquer operação destrutiva.

### 12.3 Backend Rails

- A suite principal é RSpec.
- Para alterações Rails, use o ambiente realmente configurado pelo repositório; não presuma que Ruby/Bundler estejam instalados no host.
- Prefira service objects para lógica de negócio complexa.
- Pundit é obrigatório para recursos sensíveis.
- Toda consulta multi-tenant deve preservar o escopo do tenant.
- Conversões e operações compostas que exigem consistência devem ser atômicas.
- Migrations devem ser reversíveis; não crie ou altere schema sem necessidade comprovada e autorização quando a tarefa exigir aprovação de schema.
- Após alterações, execute os testes focados primeiro; amplie para a suite adequada quando necessário.
- Não use o banco de produção para validar specs.

### 12.4 Frontend Next.js

- Preserve App Router, TypeScript e os padrões existentes do módulo.
- Use os componentes e tokens de design existentes antes de criar novas abstrações.
- Não introduza mock data ou métricas fictícias para preencher dashboards.
- Valide estados de loading, empty, error, permission e mobile.
- Para mudanças relevantes, execute `lint`, `typecheck` e testes focados; rode build quando a alteração puder afetar compilação/SSR/rotas.

### 12.5 Mobile

- Consulte `AB0-1-mobile/AGENTS.md` e `docs/guides/safe-area-guide.md`.
- Nunca use cores hardcoded quando o design system possuir token correspondente.
- Respeite safe-area e navegação `expo-router`.
- Antes de finalizar mudanças de UI, execute `npm run ui-audit` e os testes pertinentes.
