...
## SEÇÃO ESPECIAL I: ESTRUTURA GERAL DA PLATAFORMA (PRODUCT VISION & CRM DISCOVERY)

Este módulo estabelece o alinhamento estratégico do **CRM Avalia Solar** baseado nos 95 arquivos de fontes (planilhas, imagens de telas, modais e fluxos) e na engenharia reversa do sistema.

### 1. Visão de Produto e Módulos Principais
O CRM foi concebido para unificar o processo comercial de vendas de energia solar distribuída (residencial, comercial e industrial), integrando a captação digital, o dimensionamento técnico-financeiro de usinas fotovoltaicas, a gestão de relacionamento (SDR/SDR-to-AE) e a automação pós-venda.
Os módulos principais mapeados compreendem:
1. **Dashboard Comercial**: Cockpit operacional diário com agendamentos de vistorias técnicas e métricas rápidas de conversão.
2. **Leads / Prospects**: Base de qualificação de novas oportunidades captadas via landing pages ou inserção manual.
3. **Pipeline (Kanban)**: Funil visual dividido em etapas estruturadas (*Qualify*, *Pitch*, *Close*) com cálculos reativos de receita.
4. **Oportunidades (`Opportunities`)**: Projetos comerciais formalizados vinculados a Contas e Contatos.
5. **Contas / Empresas**: Organizações B2B e usinas solares de alta tensão (Grupo A).
6. **Pessoas / Contatos**: Pessoas físicas e tomadores de decisão (Sponsor, Técnico, Financeiro, Decisor).
7. **Tarefas (`Tasks`)**: Controle de obrigações com prazos, alertas e status de progresso.
8. **Atividades Comerciais (`Activities`)**: Histórico de ligações, visitas de viabilidade técnica (sombreamento) e reuniões.
9. **E-mails**: Integração nativa de comunicação (envio, recebimento, controle de abas e templates dinâmicos).
10. **Importação (`Imports`)**: Mecanismo de ingestão em massa de planilhas de bases legadas.
11. **Relatórios (`Reports`)**: Inteligência analítica para tomadas de decisão sobre conversão, metas e comissionamento.
12. **Configurações (`Settings`)**: Configuração de regras de distribuição de leads, tags, territórios geográficos e campos customizados.
13. **Permissões (RBAC)**: Matriz de controle de acessos de segurança baseada em perfis.
14. **Auditoria (`Audit Logs`)**: Rastreabilidade de ações críticas (exclusão de leads, visualização de chaves de API).
15. **Integrações**: Webhooks para landing pages, chaves de API e scripts de rastreamento integrados ao site institucional.

### 2. Matriz de Usuários e Perfis de Acesso (RBAC)
O sistema opera sob o controle de 6 perfis funcionais com permissões granulares:
*   **Admin**: Acesso irrestrito a todas as configurações, auditorias de sistema, chaves de API, exclusão de dados e regras globais de distribuição.
*   **Gestor Comercial**: Visão consolidada de todos os pipelines, relatórios de metas, reatribuição de proprietários de leads, aprovação de descontos em propostas solares e relatórios de comissionamento.
*   **Vendedor / SDR**: Visualização das suas próprias oportunidades, agendamento de atividades, registro de notas, envio de e-mails usando modelos e alteração de estágio das suas oportunidades atribuídas.
*   **CS (Customer Success)**: Visualização pós-venda para acompanhamento da instalação e homologação da usina solar junto à concessionária de energia elétrica.
*   **Financeiro**: Visualização de propostas comerciais vencidas, geração de faturas de sinal, aprovação de linhas de crédito fotovoltaico e acompanhamento do Win-Rate.
*   **Operador**: Atendimento inicial de triagem de canais integrados (chat, formulários de contato) e direcionamento automático de leads.

### 3. Requisitos Funcionais do Sistema (RF-001 a RF-005)
*   **RF-001 - Criar lead**: O sistema deve permitir cadastrar um novo lead com nome, telefone, e-mail, origem, status e responsável (atribuído). *Mapeado fisicamente nos arquivos `Leads.csv` e `lead-form.png`.*
*   **RF-002 - Converter lead em oportunidade**: O sistema deve permitir converter um lead qualificado em uma oportunidade ativa vinculada no pipeline comercial do sistema. *Mapeado em `pipeline.png` e `people-page-sumary-modal.png`.*
*   **RF-003 - Mover oportunidade entre etapas**: O usuário deve poder arrastar ou alterar uma oportunidade entre etapas predefinidas do funil Kanban. *Mapeado em `pipeline.png` e `report-funnel.png`.*
*   **RF-004 - Registrar atividade**: O sistema deve permitir registrar ligações, reuniões, e-mails, mensagens de WhatsApp, notas internas ou tarefas vinculadas a leads, contatos, contas corporativas ou oportunidades. *Mapeado em `Activities.csv`, `Emails.csv`, `Notes.csv`, `companie-page-write-note.png` e `companie-page-email-modal.png`.*
*   **RF-005 - Visualizar dashboard comercial**: O sistema deve consolidar métricas comerciais operacionais em tempo real em formato de cards de resumo, gráficos de vendas e listas de afazeres diários. *Mapeado em `sales-dashboard.png`.*

### 4. Regras de Negócio Críticas do Domínio (Regra 01 a Regra 05)
*   **Regra 01 - Lead duplicado**: Não permitir o cadastro de um lead que apresente o mesmo endereço de e-mail ou número de telefone já cadastrado no banco de dados, salvo se houver autorização explícita emitida por um usuário perfil Admin.
*   **Regra 02 - Lead qualificado**: Um lead só pode ser convertido em uma oportunidade comercial ativa caso seu cadastro possua, no mínimo: nome do contato, telefone de contato, origem declarada e um usuário responsável ativo (owner).
*   **Regra 03 - Oportunidade vencida**: Uma oportunidade sem qualquer atividade interativa registrada (ex: notas, telefonemas, e-mails) por mais de X dias (configurável pelo Gestor, padrão: 15 dias) deve ser classificada e marcada visualmente como parada/fria no pipeline Kanban.
*   **Regra 04 - Tarefa atrasada**: Toda tarefa com data de vencimento inferior à data/hora atual e status de progresso classificado como aberto deve ser imediatamente sinalizada e exibida na listagem de tarefas atrasadas da interface de trabalho diária.
*   **Regra 05 - Valor em pipeline**: O valor financeiro total do pipeline corresponde à soma simples das oportunidades abertas ponderadas individualmente de acordo com a probabilidade matemática definida para sua respectiva etapa de vendas (ex: etapa Qualify com 20% de probabilidade, etapa Pitch com 50%).

### 5. Modelagem Física de Dados da Entidade `opportunities`
A tabela física de oportunidades de vendas foi projetada no PostgreSQL para representar o coração comercial da plataforma, unindo as faturas de consumo de energia com as metas de conversão de usinas fotovoltaicas.

```sql
-- DDL PostgreSQL da Tabela de Oportunidades
CREATE TABLE opportunities (
    ...
    probability NUMERIC(5, 2) NOT NULL DEFAULT 0.00, -- Armazenado como percentual (ex: 50.00)
    expected_close_date DATE,
    status VARCHAR(50) NOT NULL DEFAULT 'Open' CHECK (status IN ('Open', 'Won', 'Lost', 'Archived')),
    lost_reason TEXT,
...
-- Indices de Performance RevOps
CREATE INDEX idx_opportunities_status_stage ON opportunities(status, stage_id);
CREATE INDEX idx_opportunities_owner ON opportunities(owner_id);
```

#### Relacionamentos da Entidade `opportunities`:
*   **Pertence a uma Empresa (`company_id`)**: Associação opcional (0..1) para projetos corporativos ou comerciais vinculados a contas B2B (`companies`/`accounts`).
*   **Pertence a um Contato (`contact_id`)**: Associação opcional (0..1) para o tomador de decisão principal da oportunidade.
*   **Pertence a um Pipeline (`pipeline_id`)**: Associação obrigatória (1) para identificar qual o funil de vendas ativo.
*   **Pertence a uma Etapa (`stage_id`)**: Associação obrigatória (1) para rastrear o progresso comercial (ex: Qualify, Pitch, Close).
*   **Possui Atividades (`activities`)**: Relacionamento de 1-para-M com registros de ligações, visitas de engenharia técnica e e-mails.
*   **Possui Tarefas (`tasks`)**: Relacionamento de 1-para-M com prazos operacionais.
...
#### A. Visão do Produto & Regras de Negócio (PRD)
*   **URL da Rota**: `/dashboard/sales/today`
*   **Objetivo da Tela**: Exibir uma visão diária, consolidada e em tempo real da operação comercial para que vendedores e gestores iniciem seus dias sabendo exatamente quais leads priorizar, quais tarefas estão atrasadas e como está o progresso financeiro das suas metas.
*   **Componentes Visuais da Interface**:
    1. **Cards de Métricas Principais (MTD)**: Exibição do volume total de leads novos, oportunidades ativas no funil, valor agregado em pipeline, receita prevista ponderada por probabilidade, taxa de conversão entre etapas e quantidade de tarefas pendentes ou em atraso.
    2. **Gráfico de Vendas (Histórico e Tendência)**: Plotagem de barras ou linhas mostrando o volume de negócios fechados e metas em relação ao planejado para o mês corrente.
    3. **Lista de Tarefas do Dia**: Painel listando as tarefas agendadas para o dia e aquelas classificadas como atrasadas (Regra 04).
    4. **Pipeline Resumido**: Mini visualização de funil mostrando a distribuição numérica dos leads pelas etapas.
    5. **Feed de Atividades Recentes**: Histórico corrido e reativo das últimas comunicações registradas (e-mails recebidos, anotações e ligações concluídas).
    6. **Barra de Filtros Temporais**: Seletores rápidos de período (Hoje, Últimos 7 dias, Este Mês, Ano Corrente) e filtros de equipe (Por SDR/Vendedor ou Visão Consolidada).
    7. **Botão de Ação Rápida Global (`+ Add New`)**: Atalho persistente no topo para criação imediata de registros inline.
*   **Dados Exibidos**:
    *   **Total de Leads**: Contagem inteira de novas oportunidades captadas no período.
    *   **Negócios Abertos**: Contagem de oportunidades com status 'Open'.
    *   **Valor em Pipeline**: Soma simples do valor bruto dos projetos fotovoltaicos ativos.
    *   **Receita Prevista (Ponderada)**: Aplicação da Regra 05 (Soma de `amount` multiplicado pelo percentual de probabilidade de sua etapa ativa).
    *   **Tarefas Atrasadas**: Contagem em destaque (cor vermelha de alta atenção) de tarefas abertas com prazo vencido (Regra 04).
    *   **Conversões por Etapa**: Percentuais de passagem entre Qualify -> Pitch -> Close.
*   **Estados Críticos da Tela (UI States)**:
    1. **Com dados (Active)**: Layout preenchido, cartões renderizados, timeline operacional e gráfico interativo funcional.
    2. **Sem dados (Empty)**: Exibição amigável do componente `EmptyState` com ilustrações vetoriais, indicando que nenhuma atividade ou lead foi registrado para o período filtrado, acompanhado de botão de ação rápida "Cadastrar Novo Lead".
    3. **Carregando (Loading)**: Exibição de animações do tipo Shimmer/Skeleton nos cards de métricas, na lista de tarefas e no contêiner do gráfico, travando interações parciais de salvamento para evitar duplicidade de requisições.
    4. **Erro (Error)**: Exibição de banner ilustrado de falha de conexão ou erro no banco de dados, contendo botão reativo "Tentar Novamente" que recarrega os dados da rota.
    5. **Sem permissão (Unauthorized)**: Bloqueio estrito de visualização de métricas consolidadas de faturamento global ou dados de outros vendedores caso o perfil do usuário ativo seja restrito (ex: Vendedor/SDR comum sem permissão de Gestor comercial). Exibe ilustração de acesso negado de segurança (Lock SVG) sugerindo contato com o Admin do sistema.

#### B. Design System & UI/UX Task List
*   **Design Tokens**:
    *   Container Grid: `grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-6 bg-slate-50 min-h-screen`.
    *   Cards de Métrica: `bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow duration-200`.
    *   Contagem de Atrasadas: `text-rose-600 font-extrabold bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-lg text-sm animate-pulse`.
    *   Botão de Ação: `bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-white font-bold py-2 px-4 rounded-xl shadow-lg transition-all`.
*   **Lista de Tarefas de UI/UX**:
    1. [ ] Implementar o grid responsivo do painel principal `/dashboard/sales/today`.
    2. [ ] Desenvolver o widget de métricas rápidas com animações de contagem numérica.
    3. [ ] Criar o componente de listagem de tarefas com separação visual de tarefas atrasadas (Regra 04) destacadas em vermelho.
    4. [ ] Codificar os estados visuais de esqueleto (*Skeleton Loading Screen*) para carregamento assíncrono do painel.

#### C. User Story, Story Points e Critérios de Aceite
*   **User Story**:
    > **Como** SDR / Vendedor do Avalia Solar,  
    > **Quero** visualizar minhas tarefas atrasadas e o valor financeiro ponderado do meu funil logo ao abrir o sistema,  
    > **Para** que eu possa organizar e priorizar minhas ligações e visitas de fechamento técnico no dia de hoje.  
    *   **Pontuação Estimada (Story Points)**: **8 SP (Épico de Visualização)**
*   **Critérios de Aceite (Gherkin)**:
    ```gherkin
    Cenário: Renderização automática de tarefas vencidas em destaque
      Dado que o vendedor acessa o painel na rota "/dashboard/sales/today"
      Quando existem 3 tarefas com vencimento anterior a hoje com status "Aberto"
      Então o card de tarefas deve exibir o contador de tarefas atrasadas com o número "3" em cor vermelha pulsante
      E listar essas 3 tarefas no topo da seção de pendências
    ```
...
*   **Componente React (TypeScript)**:
    ...
    interface SalesDashboardProps {
      userRole: 'Admin' | 'Manager' | 'SDR' | 'CS' | 'Finance' | 'Operator';
      currentUserId: string;
      onQuickLeadCreate: () => void;
    }
    ```
*   **Comportamento & Erros**: Ao carregar a tela, a aplicação dispara chamadas assíncronas paralelas via React Query para os endpoints `/api/v1/dashboard/metrics` e `/api/v1/dashboard/tasks`. Caso o usuário não tenha permissão de Gestor/Admin para ver o consolidado geral, os componentes de faturamento corporativo interceptam a requisição e exibem o estado de erro "Sem permissão" (Unauthorized), permitindo apenas a exibição das métricas pessoais do vendedor ativo.
