...
Write-Host "🤖 Iniciando reorganizacao estrutural do Hermes Agent para Avalia Solar..." -ForegroundColor Cyan
...
foreach ($Dir in $Dirs) {
    ...
    if (-not (Test-Path $TargetDir)) {
        ...
        Write-Host "   [Dir] Criado diretorio: hermes-agent/$Dir" -ForegroundColor Gray
    }
}

# 2. Migrar e Consolidar Documentos
Write-Host "Movendo e consolidando documentos estrategicos..." -ForegroundColor Green

# Source: .planning/skills/
$DocsToMove = @(
    @{ Src = "HERMES_GROWTH_AUTOMATION_MAP.md"; Dest = "docs\growth-automation-map.md" },
    @{ Src = "README-AVALIASOLAR-MOBILIDADE.md"; Dest = "docs\mobility-alignment.md" },
    @{ Src = "roadmap-avaliasolar-mobilidade.md"; Dest = "docs\roadmap.md" }
)

foreach ($Doc in $DocsToMove) {
    $SrcPath = "$RootPath\.planning\skills\$($Doc.Src)"
    if (Test-Path $SrcPath) {
        Move-Item -Path $SrcPath -Destination "$HermesDir\$($Doc.Dest)" -Force
        Write-Host "   [OK] Movido: $($Doc.Src) -> hermes-agent/$($Doc.Dest)"
    }
}

# 3. Migrar e Reorganizar Custom Skills
Write-Host "Reorganizando Custom Skills cognitivas e scripts TS..." -ForegroundColor Green
...
if (Test-Path "$RootPath\.planning\skills\utils.ts") {
    ...
    Write-Host "   [OK] Movido: utils.ts -> hermes-agent/skills/utils/utils.ts"
}
...
if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\linkedin-regional-prospector.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\linkedin-regional-prospector.ts" -Destination "$HermesDir\skills\linkedin-prospector\scripts\linkedin-regional-prospector.ts" -Force
}
Write-Host "   [OK] Reorganizado: hermes-linkedin-prospector -> hermes-agent/skills/linkedin-prospector/"
...
Write-Host "   [OK] Reorganizado: hermes-competitor-listening -> hermes-agent/skills/competitor-listening/"
...
if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\gmail-classifier.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\gmail-classifier.ts" -Destination "$HermesDir\skills\inbox-triager\scripts\gmail-classifier.ts" -Force
}
Write-Host "   [OK] Reorganizado: hermes-inbox-triager -> hermes-agent/skills/inbox-triager/"

# Solar Mobility Prospeo
if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\prospeo-lead-export.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\prospeo-lead-export.ts" -Destination "$HermesDir\skills\solar-mobility-leads\prospeo-lead-export.ts" -Force
    Write-Host "   [OK] Reorganizado: prospeo-lead-export.ts -> hermes-agent/skills/solar-mobility-leads/prospeo-lead-export.ts"
}

# 4. Migrar Outros Scripts Auxiliares
Write-Host "Reorganizando scripts operacionais e configuracoes..." -ForegroundColor Green

if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\instagram-keyword-reply.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\instagram-keyword-reply.ts" -Destination "$HermesDir\scripts\instagram-keyword-reply.ts" -Force
}
if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\nutshell-lead-enrichment.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\nutshell-lead-enrichment.ts" -Destination "$HermesDir\scripts\nutshell-lead-enrichment.ts" -Force
}
if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\abandoned-checkout-recovery.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\abandoned-checkout-recovery.ts" -Destination "$HermesDir\scripts\abandoned-checkout-recovery.ts" -Force
}
if (Test-Path "$RootPath\.planning\skills\skills\avaliasolar\verify-credentials-avaliasolar.ts") {
    Move-Item -Path "$RootPath\.planning\skills\skills\avaliasolar\verify-credentials-avaliasolar.ts" -Destination "$HermesDir\scripts\verify-credentials-avaliasolar.ts" -Force
}

# Config / env.example
if (Test-Path "$RootPath\.env.example") {
    Copy-Item -Path "$RootPath\.env.example" -Destination "$HermesDir\config\.env.example" -Force
    Write-Host "   [OK] Copiado: .env.example -> hermes-agent/config/.env.example"
}

# Limpeza de pastas vazias antigas
if (Test-Path "$RootPath\.planning\skills\skills") {
    Remove-Item -Path "$RootPath\.planning\skills\skills" -Recurse -Force | Out-Null
}
...
Write-Host "Reorganizacao concluida com sucesso!" -ForegroundColor Yellow
