...
export default function CreateOpportunityDialog({ open, onOpenChange, onSuccess }: CreateOpportunityDialogProps) {
  ...
  const handleCreateNewOpportunity = async (e: React.FormEvent) => {
    ...
    createMutation.mutate(payload, {
      ...
      onError: (err: any) => {
        setCreateError(err.message || 'Erro ao criar oportunidade comercial');
      },
    });
  };
  ...
  return (
      ...
      footer={
        <>
          <Button
            ...
            onClick={handleClose}
            disabled={isCreating}
            className="h-11 text-xs font-semibold text-slate-600 hover:text-slate-900 px-5"
          >
            Cancelar
          </Button>
          <Button
            ...
            disabled={isCreating}
            className="h-11 px-6 text-xs font-bold bg-indigo-900 hover:bg-indigo-950 text-white rounded-xl shadow-md transition-all duration-150"
          >
            {isCreating ? (
              <>
    ...
    >
      <form id="create-opportunity-form" onSubmit={handleCreateNewOpportunity} className="space-y-6 font-sans">
        {createError && (
          <div className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50/90 p-4 text-xs font-semibold text-red-800 shadow-2xs">
            <AlertCircle className="mt-0.5 h-4 w-4 flex-shrink-0 text-red-600" />
            <div className="space-y-0.5">
              <p className="font-bold text-red-900">Falha ao salvar oportunidade</p>
        ...
        <CRMFormRow cols={2}>
          ...
          <CRMFormField label="Empresa (Account)" required>
            {!isInlineAccount ? (
              ...
              <div className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-4 space-y-3 shadow-2xs">
                ...
                <Input
                  ...
                  className="h-10 border-slate-300 text-xs bg-white rounded-lg px-3.5"
                />
                <Input
          ...
          <CRMFormField label="Contato Principal">
            {!isInlineContact ? (
              ...
              <div className="rounded-xl border border-sky-200 bg-sky-50/50 p-4 space-y-3 shadow-2xs">
                <div className="flex items-center justify-between">
        ...
        <CRMFormField label="Nome da Oportunidade" required>
          <Input
            ...
            className="h-11 text-xs font-medium border-slate-300 focus:border-indigo-600 rounded-xl px-4 shadow-2xs"
            disabled={isCreating}
            required
          />
        </CRMFormField>
        ...
        <CRMFormRow cols={2}>
          <CRMFormField label="Estágio Inicial">
            <Select value={newStage} onValueChange={setNewStage} disabled={isCreating}>
              <SelectTrigger className="h-11 border-slate-300 text-xs rounded-xl px-4 bg-white shadow-2xs">
                <SelectValue placeholder="Selecione estágio" />
              </SelectTrigger>
              <SelectContent className="rounded-xl shadow-xl">
                {stages.filter((s: any) => s.key !== 'won' && s.key !== 'lost').map((s: any) => (
                  <SelectItem key={s.key} value={s.key} className="text-xs py-2.5 px-3">
                    {s.name || s.label}
                  </SelectItem>
                ))}
              </SelectContent>
