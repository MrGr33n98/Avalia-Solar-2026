...
jest.mock('@/components/ComparisonToggleButton', () => ({ company }: { company: { id: number } }) => (
  <button data-testid={`compare-${company.id}`}>Compare</button>
));
jest.mock('@/lib/feature-access', () => ({ hasPaidPlan: (company: { has_paid_plan?: boolean }) => Boolean(company.has_paid_plan) }));
jest.mock('@/lib/lead-engine', () => ({ openLeadModal: jest.fn() }));

const company = (id: number, paid: boolean) => ({
  id, name: `Empresa ${id}`, slug: `empresa-${id}`, city: 'São Paulo', state: 'SP',
  status: 'active', description: '', website: '', phone: '', address: '',
  created_at: '', updated_at: '', has_paid_plan: paid,
} as any);
...
describe('CategoryCompaniesTable ações', () => {
  it('reserva slot comercial e mantém Compare alinhado para capabilities mistas', () => {
    ...
    expect(screen.getAllByTestId(/compare-/)).toHaveLength(2);
