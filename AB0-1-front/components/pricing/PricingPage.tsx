...
function normalizePlanSlug(
  plan: Partial<BillingPlan> & { plan_tier?: string },
  fallbackIndex = 0
): PlanSlug {
  const explicitSlug = plan.slug || plan.plan_tier;
  if (
    explicitSlug === 'free' ||
    explicitSlug === 'essential' ||
    explicitSlug === 'pro' ||
    explicitSlug === 'enterprise'
  ) {
  ...
  if (normalizedName.includes('essential') || normalizedName.includes('essencial'))
    return 'essential';
...
export default function PricingPage() {
    ...
    async function loadPricingData() {
      ...
      try {
        ...
        try {
          ...
          console.warn(
            '[PricingPage] Falha ao carregar planos da API, usando catálogo estático como fallback:',
            err
          );
        }

        const apiPlansBySlug = apiPlans.reduce<Partial<Record<PlanSlug, BillingPlan>>>(
          (acc, apiPlan, index) => {
        ...
        const displayPlans = pricingPlans
          .map((staticPlan, index) => {
            ...
            return {
              ...
              price_cents: apiPlan?.price_cents ?? 0,
              price_formatted: apiPlan?.price_formatted || staticPlan.priceLabel,
              price_label:
                apiPlan?.price_label || apiPlan?.price_formatted || staticPlan.priceLabel,
        ...
        setError(
          'Ocorreu um erro ao carregar os planos de faturamento. Por favor, tente novamente mais tarde.'
        );
      } finally {
        setLoading(false);
      }
  ...
  const handlePlanCta = async (plan: BillingPlan) => {
    ...
    if (!user?.company_id) {
      router.push(
        '/select-company?reason=billing_required&plan=' + plan.slug + '&return_to=/pricing'
      );
      return;
    }
    ...
    try {
      ...
      if (plan.slug === 'essential') {
        ...
        if (plan.stripe_price_id_monthly) {
          ...
          console.info(
            '[PricingPage] Checkout do plano Essencial encaminhado para o dashboard local.'
          );
  ...
  const handleEnterpriseLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setModalSuccessMessage(
        'Sua solicitação de plano Enterprise foi enviada com sucesso! Nosso time comercial entrará em contato em breve.'
      );
      ...
      setModalError(
        error?.message || 'Ocorreu um erro ao enviar sua solicitação. Por favor, tente novamente.'
      );
    } finally {
  ...
  const checkIsCurrentPlan = (planSlug: PlanSlug) => {
    ...
    return (
      ...
      ['active', 'trialing', 'past_due', 'enterprise_lead'].includes(subscription.status)
    );
  };
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#F5F8FC] pb-12 font-sans antialiased text-slate-800">
      ...
      <section className="bg-white border-b border-slate-200/60 py-8">
        <div className="max-w-[1160px] mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <div className="space-y-1">
            <strong className="block text-[15.5px] font-extrabold text-slate-900">
          ...
          <div className="space-y-1">
            ...
            <span className="text-[13px] text-slate-500 font-medium">
              presente nas principais capitais
            </span>
          </div>
          <div className="space-y-1">
            <strong className="block text-[15.5px] font-extrabold text-slate-900">
        ...
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            <span className="text-xs font-bold uppercase tracking-[.1em] text-amber-600">
              Como você ganha clientes
            </span>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
              Da busca ao orçamento fechado
            </h2>
            <p className="text-[17px] text-slate-500 font-medium">
              Seu perfil na Avalia Solar não é um cartão de visitas. É um funil completo de
              conversão.
            </p>
          </div>
          <div className="flex flex-col md:flex-row items-stretch justify-center gap-4 max-w-[980px] mx-auto">
            {[
              {
                num: 1,
                title: 'Consumidor pesquisa',
                desc: 'empresas de energia solar na sua cidade',
              },
              { num: 2, title: 'Encontra seu perfil', desc: 'com destaque e sem concorrentes' },
              {
                num: 3,
                title: 'Ganha confiança',
                desc: 'avaliações reais, fotos, FAQ e materiais',
              },
              ...
              <div key={step.num} className="flex flex-col md:flex-row items-center flex-1">
                <div className="flex-1 bg-slate-50 border border-slate-200/60 rounded-xl p-5 text-center flex flex-col items-center justify-start min-h-[170px] shadow-sm hover:shadow transition-shadow w-full">
                  ...
                  <strong className="block text-[14.5px] font-black text-slate-900 mb-1">
                    {step.title}
                  </strong>
          ...
          <p className="text-center mt-8 text-[15px] text-slate-500 font-medium max-w-2xl mx-auto leading-relaxed">
            Planos pagos removem as distrações desse caminho:{' '}
            <strong className="text-slate-950 font-bold">
              sem anúncios de concorrentes no seu perfil
            </strong>
            , a atenção fica 100% na sua empresa.
          </p>
        </div>
      </section>
      ...
      <section className="py-20 bg-slate-50 border-b border-slate-200/60">
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            ...
            <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
              Quanto vale um cliente a mais por mês?
            </h2>
            <p className="text-[17px] text-slate-500 font-medium">
              Arraste os valores da sua realidade e veja o que poucos contatos extras já
              representam.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-[980px] mx-auto items-stretch">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-sm font-bold text-slate-900">
                  ...
                  <span className="text-amber-600 text-base font-extrabold">
                    R$ {roiTicket.toLocaleString('pt-BR')}
                  </span>
                </div>
                <input
                  type="range"
                  ...
                  className="min-h-11 w-full accent-amber-500 h-2 bg-slate-100 rounded-lg appearance-none cursor-pointer"
                />
              </div>
            ...
            <div className="bg-slate-950 text-white rounded-2xl p-8 flex flex-col justify-between shadow-lg">
              ...
              <p className="text-[11px] text-slate-500 leading-relaxed mt-6">
                Simulação ilustrativa: o resultado real depende da sua região, do seu perfil e da
                sua taxa de fechamento. Mesmo fechando apenas 1 instalação a mais a cada poucos
                meses, o plano já se paga.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            ...
            <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
              Não é um perfil premium. É crescimento comercial.
            </h2>
            <p className="text-[17px] text-slate-500 font-medium">
              Empresas de energia solar não compram banner e FAQ — compram mais vendas, mais
              autoridade e menos CAC.
            </p>
          </div>
            ...
            {[
              {
                ...
                desc: 'Botão de WhatsApp e CTAs no seu perfil, conectando você a quem já está pronto para pedir orçamento.',
              },
              {
                title: 'Mais autoridade',
                desc: 'Perfil verificado, avaliações reais e destaque nas buscas — sua empresa parece (e é) a escolha segura.',
              },
              {
                title: 'Menos CAC',
                desc: 'Sem comissão por lead e sem depender só de anúncios pagos. Um cliente fechado já paga o plano por anos.',
              },
              {
                title: 'Mais inteligência',
                desc: 'Relatórios de visitas e contatos para você saber exatamente quanto o portal gera para o seu negócio.',
              },
            ].map((b, idx) => (
              <div
                key={idx}
                className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-start"
              >
                ...
                <h3 className="text-[16.5px] font-black text-slate-950 mb-2 leading-tight">
                  {b.title}
                </h3>
      ...
      >
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            ...
            <p className="text-[17px] text-slate-500 font-medium">
              Comece grátis e evolua quando quiser. Todos os planos pagos removem concorrentes do
              seu perfil.
            </p>
          </div>
          ...
          <p className="text-center mt-8 text-slate-500 text-xs font-semibold">
            <strong className="text-slate-900 font-bold">Todos os planos pagos:</strong> sem
            fidelidade · cancele quando quiser · ativação em minutos · pagamento seguro
          </p>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            ...
            <p className="text-[17px] text-slate-500 font-medium">
              Encontre o seu cenário abaixo e vá direto ao plano certo.
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              {
                scenario: '"Tenho equipe comercial e preciso integrar com meu CRM"',
                action: '→ Enterprise',
              },
            ].map((card, idx) => (
              <div
                key={idx}
                className="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-5 hover:border-slate-350 transition-colors flex flex-col justify-between shadow-sm"
              >
                <p className="text-[14.5px] text-slate-600 font-medium leading-relaxed mb-4">
                  {card.scenario}
                </p>
      ...
      <section className="py-12 bg-white border-t border-slate-100">
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            ...
            <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
              Compare os recursos lado a lado
            </h2>
            <p className="text-[17px] text-slate-500 font-medium">
              As principais diferenças que impulsionam os resultados comerciais da sua empresa.
            </p>
          </div>
        ...
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-12 items-center">
            ...
            <div className="space-y-6">
              ...
              <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
                Saiba exatamente quanto o portal gera para você
              </h2>
              <p className="text-slate-500 text-[16px] font-medium leading-relaxed">
                Nada de investir no escuro. Acompanhe o desempenho do seu perfil e prove o retorno
                do seu investimento.
              </p>
              ...
              <Button
                asChild
                size="lg"
                className="bg-slate-900 hover:bg-slate-800 text-white rounded-full font-bold px-8 h-11 text-xs shadow-md"
              >
                ...
                <div className="space-y-2">
                  <div className="flex items-end gap-3 h-28 px-2 border-b border-slate-100">
                    {[35, 48, 42, 60, 55, 78, 92].map((height, i) => (
                      <div
                        key={i}
                        className="flex-1 bg-gradient-to-t from-amber-400 to-amber-500 rounded-t-sm"
                        style={{ height: `${height}%` }}
                      />
                    ))}
                  </div>
      ...
      <section className="py-20 bg-slate-50 border-t border-b border-slate-200/60">
        <div className="max-w-[1160px] mx-auto px-6">
          <div className="text-center max-w-[720px] mx-auto mb-12 space-y-3">
            ...
            <h2 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
              Teste sem medo de se arrepender
            </h2>
            <p className="text-[17px] text-slate-500 font-medium">
              Você não precisa de contrato longo nem burocracia para começar a receber contatos.
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              {
                ...
                desc: 'Cancele quando quiser, sem multa e sem letra miúda.',
              },
              {
                title: 'Ativação em minutos',
                desc: 'Assinou, seu perfil já aparece com os recursos do plano.',
              },
              {
                title: 'Pagamento seguro',
                desc: 'Ambiente 100% protegido para seus dados e cobranças.',
              },
              {
                title: 'Suporte humano',
                desc: 'Atendimento especializado de gente de verdade, em português.',
              },
            ].map((g, idx) => (
              <div
                key={idx}
                className="bg-white border border-slate-200/80 rounded-2xl p-6 text-center shadow-sm"
              >
                ...
                <h3 className="text-base font-black text-slate-950 mb-2 leading-tight">
                  {g.title}
                </h3>
        ...
        <div className="container mx-auto px-4 md:px-6 max-w-[1160px]">
          <div className="mb-12 text-center max-w-2xl mx-auto space-y-3">
            ...
            <p className="text-sm sm:text-base text-slate-600 font-medium leading-relaxed">
              Posicione sua marca onde os compradores decidem seus parceiros e integradores de
              energia solar.
            </p>
          </div>
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-slate-800/10 bg-slate-950 p-6 sm:p-8 text-white shadow-xl flex flex-col justify-between">
              ...
              <div className="space-y-4">
                ...
                <p className="text-sm leading-relaxed text-slate-350 font-medium">
                  Coloque sua marca diante de centenas de decisores e consumidores que já estão
                  pesquisando ativamente empresas, produtos e soluções de energia solar no Brasil.
                </p>
              ...
              <div className="pt-6">
                <Button
                  asChild
                  size="lg"
                  className="bg-[#B7F000] hover:bg-[#A3D600] text-slate-950 font-bold border-0 shadow-lg shadow-[#B7F000]/10 h-11 w-full rounded-full text-xs"
                >
            ...
            <div className="rounded-[2rem] border border-brand-blue/15 bg-gradient-to-br from-brand-blue/5 via-white to-white p-6 sm:p-8 shadow-sm flex flex-col justify-between">
              <div className="space-y-4">
                ...
                <p className="text-sm leading-relaxed text-slate-600 font-medium">
                  Precisa de proposta personalizada, integrações robustas via webhook com seu CRM ou
                  faturamento customizado? Converse agora com nosso time comercial.
                </p>
                ...
                <div className="flex items-center gap-3 pt-3">
                  <div className="flex -space-x-2.5 overflow-hidden">
                    {[
                      ...
                      <div
                        key={index}
                        className="inline-block h-8 w-8 rounded-full ring-2 ring-white overflow-hidden bg-slate-200"
                      >
              ...
              <div className="pt-6">
                <Button
                  asChild
                  size="lg"
                  className="bg-slate-900 hover:bg-slate-800 text-white font-bold border-0 shadow-lg h-11 w-full rounded-full text-xs"
                >
      ...
      {/* ── 12. FAQ ────────────────── */}
      <section className="mx-auto max-w-[1160px] space-y-8 px-6 py-12">
            ...
            >
              ...
              <div className="mb-6">
                ...
                <p className="mt-2 text-sm leading-relaxed text-slate-500">
                  Nos diga um pouco sobre sua operação e necessidades para que possamos formatar a
                  proposta ideal para sua empresa.
                </p>
              </div>
