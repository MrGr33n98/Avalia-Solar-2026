...
export function AddUserSolutionModal({ open, onOpenChange, onAdd }: AddUserSolutionModalProps) {
  ...
  const reset = () => {
    setName('');
    setCategory('Energia Solar');
    setType('technology');
  };
  useEffect(() => {
    if (open) track('reviewer_solution_add_opened', { source: 'solution_modal' });
  }, [open]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl sm:max-w-lg bg-white p-6">
        <DialogHeader>
          <DialogTitle>Adicionar solução sustentável</DialogTitle>
          <DialogDescription>
            Cadastre solução realmente utilizada. Validação pode ocorrer depois.
          </DialogDescription>
        </DialogHeader>
