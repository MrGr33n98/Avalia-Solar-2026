    ...
    >
      {isOpen && !isBlocked && (
        <div className={cn(
          ...
          isPwa || isMobile ? 'fixed bottom-[calc(4.5rem+var(--safe-area-inset-bottom))] left-1/2 h-[min(78dvh,620px)] max-h-[620px] min-h-[min(420px,calc(100dvh-7rem))] w-[calc(100dvw-24px)] max-w-[420px] -translate-x-1/2 rounded-2xl pb-0' : ''
        )}>
          {/* Header */}
          ...
          <form onSubmit={handleSend} className="p-3 border-t border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex items-center space-x-2">
            ...
            {isLoading && (
              <button
                ...
                className="rounded-xl border border-zinc-300 px-3 py-2 text-xs font-medium text-zinc-700 transition-colors hover:bg-zinc-50"
