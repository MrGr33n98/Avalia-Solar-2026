    ...
    const specPills =
      product.specs && product.specs.length > 0
        ? product.specs.slice(0, 4).map((spec, idx) => (
            <span
              key={idx}
              className="inline-flex items-center rounded-md bg-[#f1f5f9] px-2.5 py-1 text-xs font-bold text-[#475569] shadow-sm"
            >
              {spec.value}
            </span>
          ))
    ...
    return (
      <>
        ...
        >
          <Card
            ref={cardRef}
            className="w-full flex flex-col md:flex-row overflow-hidden border-slate-200/80 hover:border-blue-500/20 hover:shadow-lg transition-all duration-300 bg-white rounded-2xl relative"
          >
            {/* Left: Image Box */}
            ...
            <div className="flex-1 p-5 flex flex-col justify-center gap-3 relative pr-6">
              <div className="space-y-2">
                ...
                {specPills && <div className="flex flex-wrap gap-2 pt-2">{specPills}</div>}
              </div>
            </div>
            ...
            <div className="w-full md:w-64 border-t md:border-t-0 md:border-l border-slate-100 p-5 bg-white flex flex-col justify-center items-end gap-3 text-right relative pr-6">
              ...
              {ratingAvg !== undefined && ratingAvg !== null ? (
                <div className="flex items-center gap-1 text-xs font-bold text-slate-700 w-full justify-end">
                  <span className="text-slate-800 font-extrabold">
                    {ratingAvg.toLocaleString('pt-BR', {
              ...
              <div className="space-y-1 w-full mt-1">
                {priceAvailable ? (
                  ...
                  <div className="text-sm font-bold text-slate-500">Preço sob consulta</div>
                )}
              </div>
              ...
              <Button
                asChild
                variant="outline"
                className="h-10 w-full rounded-lg border border-blue-600 bg-white text-xs font-bold text-blue-600 hover:bg-blue-50/50 hover:text-blue-700 hover:border-blue-700 transition-all shadow-sm"
              >
                <Link
                  href={friendlyUrl}
                  onClick={() =>
  ...
  return (
    <>
      ...
      >
        <Card
          ref={cardRef}
          className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
        >
          {/* Image Section */}
          ...
          <CardContent className="flex flex-grow flex-col gap-2 p-4 pt-3">
            ...
            <Link href={friendlyUrl} className="block transition-colors group-hover:text-blue-700">
              <h2
                itemProp="name"
                className="line-clamp-2 min-h-[2.5rem] text-sm font-bold leading-snug text-slate-900"
              >
                {product.name}
              </h2>
            </Link>
            ...
            <div className="flex items-center gap-1 text-[11px] font-semibold text-slate-500 mt-0.5">
              ...
              <button
                type="button"
                aria-label="Mais informações sobre a marca"
