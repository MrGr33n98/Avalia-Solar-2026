...
export function CompanyCategoryPicker({
  ...
  const renderCategoryNode = (node: CategoryTreeNode, depth: number = 0) => {
    ...
    return (
      <div key={node.id} className="space-y-0.5">
        <div
          className={`flex items-center justify-between py-1 px-1.5 md:py-1.5 md:px-2 rounded-md cursor-pointer transition-all duration-200 select-none ${
            isSelected
        ...
        >
          <div className="flex items-center gap-2 min-w-0">
            <Checkbox
              ...
              className="h-3.5 w-3.5 rounded-sm data-[state=checked]:bg-blue-600 data-[state=checked]:border-blue-600 shrink-0"
              onClick={(e) => e.stopPropagation()}
            />
            <label
              htmlFor={`picker-cat-${node.id}`}
              className={`text-[11px] md:text-xs cursor-pointer transition-colors leading-snug truncate ${
                depth === 0 ? 'font-bold text-slate-800' : 'text-slate-600 font-medium'
  ...
  return (
    ...
    >
      {/* Search Header */}
      <div className="p-3 md:p-4 border-b border-slate-100 space-y-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full h-9 pl-9 pr-9 py-1.5 text-xs bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all placeholder:text-slate-400"
          />
          {searchTerm && (
            <button
              type="button"
      ...
      {/* Tree Content */}
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5 md:p-4 md:space-y-3 min-h-[250px] max-h-[320px]">
        {loading ? (
