...
export function NewsFeedCard({ item }: { item: FeedItem }) {
  ...
  const open = () =>
    track('news_open', {
      ...
      view: item.ranking_metadata?.mode,
    });
  const impression = useCallback(
    () =>
      track('news_impression', {
        feed_item_id: item.id,
        subject_id: subject.id,
        subject_type: item.type,
        actor_id: item.actor.id,
    ...
    [item.id, item.type, item.actor.id, item.ranking_metadata?.mode, subject.id]
  );
  useEffect(() => {
    ...
    <FeedCardFrame reason={item.recommendation_reason?.label} itemId={item.id} itemType={item.type}>
      <div ref={cardRef}>
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-primary">
          Avalia Solar Notícias
        </p>
        <p className="mt-2 text-xs text-muted-foreground">
          {subject.category || 'Energia'} · {subject.reading_time_minutes || 3} min de leitura
        </p>
        <h2 className="mt-2 text-xl font-bold tracking-tight">{subject.title}</h2>
        {subject.excerpt && (
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{subject.excerpt}</p>
        )}
        {subject.source_url && (
          <Link
