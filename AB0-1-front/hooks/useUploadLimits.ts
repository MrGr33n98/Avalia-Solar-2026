...
export function useUploadLimits(companyId: string) {
  const queryClient = useQueryClient();

  const { data: limits, isLoading, error, refetch } = useQuery<UploadLimits>({
    queryKey: ['upload-limits', companyId],
    queryFn: async () => {
      const response = await fetch(`/api/v1/companies/${companyId}/upload_limits`, {
        credentials: 'include',
      });
      if (!response.ok) throw new Error('Failed to fetch upload limits');
  ...
  const checkUploadMutation = useMutation({
    mutationFn: async ({ type, count = 1 }: { type: 'images' | 'videos' | 'projects'; count?: number }) => {
      const response = await fetch(
        `/api/v1/companies/${companyId}/upload_limits/check?type=${type}&count=${count}`,
        { credentials: 'include' }
      );
      return response.json();
    },
  ...
  const canUpload = useCallback(
    (type: 'images' | 'videos' | 'projects', count = 1) => {
      if (!limits) return false;
      const current = limits.usage[type];
      const limit = limits.limits[type];
      if (limit === null) return true;
      return current + count <= (limit || 0);
    },
    [limits]
  );

  const usagePercentage = useCallback(
    (type: 'images' | 'videos' | 'projects') => {
      if (!limits) return 0;
      return limits.percentages[type] || 0;
    },
    [limits]
  ...
  return {
    ...
    error,
    refetch,
    checkUpload: checkUploadMutation.mutateAsync,
    incrementUsage: incrementUsageMutation.mutateAsync,
    canUploadImage: (count = 1) => canUpload('images', count),
    canUploadVideo: (count = 1) => canUpload('videos', count),
    canCreateProject: (count = 1) => canUpload('projects', count),
    usagePercentage,
    isNearLimit: (type: 'images' | 'videos' | 'projects', threshold = 0.8) => {
      if (!limits) return false;
      return limits.near_limits[type] || usagePercentage(type) >= threshold * 100;
    },
  };
