                    ...
                    <div key={lead.id} className="flex items-start gap-3 p-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-xs font-semibold text-slate-800">{lead.name || 'Contato sem nome'}</p>
                          <span className={cn('shrink-0 rounded-md px-2 py-0.5 text-[10px] font-semibold', statusTone(lead.wizard_status))}>
                        ...
                        <p className="mt-1 line-clamp-1 text-[11px] text-slate-600" title={lead.product_vertical || lead.project_profile || lead.message}>
                          {lead.product_vertical || lead.project_profile || lead.message || 'Solicitação de orçamento'}
                        </p>
                        <div className="mt-2 flex items-center justify-between gap-2">
                          <span className="rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                            {lead.utm_source || lead.referrer_host || 'Portal'}
                          </span>
                          <span className="text-[10px] text-slate-400">{formatRelativeDate(lead.created_at)}</span>
                        </div>
                      </div>
                    </div>
                  ))}
