    ...
    <div className="space-y-12">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="flex gap-4 p-2 bg-white/[0.02] border border-white/5 rounded-2xl backdrop-blur-sm">
           ...
           <Button variant="ghost" className="h-10 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white">
              Sugerir Novo Selo
           </Button>
        </div>
      </div>
      ...
      {badges.length === 0 ? (
        <Card className="clay-precision bg-white/[0.01] border-dashed border-2 border-white/5 rounded-[3rem] overflow-hidden">
          <CardContent className="flex flex-col items-center justify-center py-32 text-center">
            <div className="h-24 w-24 rounded-[2rem] bg-white/5 flex items-center justify-center mb-8 relative">
               <Award className="h-10 w-10 text-white/20" />
               <div className="absolute inset-0 bg-primary/5 blur-2xl rounded-full" />
            </div>
            <h3 className="text-xl font-black uppercase tracking-tighter text-white mb-2">Protocolo de Distinção Vazio</h3>
            <p className="text-sm text-white/30 font-medium max-w-xs mb-8">
              Sua empresa ainda não possui selos de distinção atribuídos pela nossa inteligência de mercado.
            </p>
            <Button variant="outline" className="h-12 border-white/10 bg-white/5 hover:bg-white/10 text-white font-black uppercase tracking-widest text-[10px] rounded-xl px-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <AnimatePresence>
            {badges.map((badge, index) => (
              ...
              >
                <Card className="clay-precision bg-[#002B4D]/50 backdrop-blur-xl border-none rounded-[2.5rem] overflow-hidden group transition-all duration-500 hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-2 h-full flex flex-col">
                  {/* Visual Preview */}
                  <div className="h-56 relative flex items-center justify-center overflow-hidden border-b border-white/5 bg-gradient-to-br from-white/[0.03] to-transparent">
                  ...
                  <CardHeader className="p-8 pb-4">
                    <CardTitle className="text-2xl font-black text-white uppercase tracking-tighter mb-2 group-hover:text-primary transition-colors">
                      {badge.name}
                    </CardTitle>
                    <CardDescription className="text-sm font-medium text-white/40 leading-relaxed min-h-[48px] line-clamp-3">
                  ...
                  <CardContent className="p-8 pt-0 mt-auto space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="relative group/snippet">
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          className="absolute right-2 top-2 h-10 w-10 bg-white/5 hover:bg-primary hover:text-white rounded-xl text-white/30 transition-all opacity-0 group-hover/snippet:opacity-100"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Button 
                        variant="outline" 
                        size="sm" 
                        className="h-11 rounded-2xl border-white/5 bg-white/[0.02] hover:bg-white/[0.05] text-[10px] font-black uppercase tracking-widest text-white/60 hover:text-white"
                        onClick={() => copyToClipboard(badge.image_url || '', badge.id, 'url')}
                      >
                        <Activity className="h-3.5 w-3.5 mr-2" />
                        Source Link
                      </Button>
                      <Button asChild size="sm" className="h-11 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-primary text-white text-[10px] font-black uppercase tracking-widest transition-all">
      ...
      <div className="p-8 rounded-[2.5rem] bg-gradient-to-r from-primary/10 to-transparent border border-white/5 relative overflow-hidden">
         <div className="absolute top-0 right-0 p-8 opacity-10">
            <Info className="h-24 w-24 text-primary" />
         </div>
         <div className="max-w-2xl relative z-10">
            <h4 className="text-base font-black text-white uppercase tracking-tight mb-2">Alavancagem de Autoridade</h4>
            <p className="text-sm text-white/50 leading-relaxed font-medium">
              A exibição destes ativos em seu site institucional aumenta em média <span className="text-white font-black underline decoration-primary">32% a taxa de conversão</span> de leads orgânicos, ao reduzir a fricção de confiança na fase de descoberta.
            </p>
         </div>
      </div>
    </div>
