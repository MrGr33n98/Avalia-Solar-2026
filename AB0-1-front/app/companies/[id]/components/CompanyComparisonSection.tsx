...
export default function CompanyComparisonSection({ currentCompany }: CompanyComparisonSectionProps) {
  ...
  return (
    <section className="mt-20 border-t border-slate-200 bg-slate-50/30 -mx-4 px-4 md:-mx-8 md:px-8 py-20">
      <div className="max-w-[1280px] mx-auto">
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-100 text-blue-700 text-[10px] font-black uppercase tracking-wider mb-4">
              <Scale className="h-3.5 w-3.5" /> Comparativo de Mercado
            </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4">
            <div className="p-10 hidden md:flex flex-col justify-end bg-slate-50/40 border-r border-slate-100">
            ...
            <AnimatePresence mode="popLayout">
              {displayedCompanies.map((company, idx) => (
                <motion.div 
                  ...
                  className={cn(
                    "p-8 flex flex-col items-center text-center relative border-b md:border-b-0 md:border-r border-slate-100 last:border-r-0",
                    idx === 0 && "bg-blue-50/20"
                  )}
                >
                  {idx > 0 && (
                  ...
                  <Button asChild variant={idx === 0 ? "default" : "outline"} className="w-full rounded-2xl font-black h-11 transition-all hover:scale-[1.02]">
                    <Link href={`/companies/${company.slug}`}>Ver perfil completo</Link>
                  </Button>
                </motion.div>
              ))}
            </AnimatePresence>
          ...
          {/* Footer Row */}
          <div className="grid grid-cols-1 md:grid-cols-4 bg-slate-50/30">
            <div className="p-8 hidden md:block border-r border-slate-100"></div>
            {displayedCompanies.map((company, idx) => (
              <div key={`cta-${company.id}`} className={cn(
                ...
                <Button 
                  ...
                  onClick={() => {
                    const leadEvent = new CustomEvent('open-lead-modal', { 
                      detail: { preferredCompanyId: company.id, source: 'comparison-section' } 
                    });
            ...
            {Array.from({ length: 3 - displayedCompanies.length }).map((_, i) => (
              <div key={`empty-cta-${i}`} className="p-8 border-r border-slate-100 last:border-r-0 hidden md:block"></div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-12 text-center">
          <Button asChild variant="link" className="text-slate-400 hover:text-blue-600 transition-colors">
            <Link href="/compare" className="flex items-center gap-2 font-black text-sm uppercase tracking-widest">
              Ver comparação detalhada <ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
        </div>
      </div>
...
function ComparisonRow({ 
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 group hover:bg-slate-50/50 transition-colors">
      <div className="p-8 flex items-center gap-4 bg-slate-50/10 md:border-r border-slate-100">
      ...
      {companies.map((company, idx) => (
        <div key={`val-${company.id}`} className={cn(
          "p-8 flex items-center justify-center md:justify-start md:border-r border-slate-100 last:border-r-0",
          idx === 0 && "bg-blue-50/10"
        )}>
          {value(company)}
        </div>
      ...
      {Array.from({ length: 3 - companies.length }).map((_, i) => (
        <div key={`empty-val-${i}`} className="p-8 md:border-r border-slate-100 last:border-r-0 hidden md:block"></div>
      ))}
    </div>
  );
}
