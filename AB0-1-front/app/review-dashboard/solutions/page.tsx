...
import { Zap, Sun, Battery, Leaf, Plus, CheckCircle2 } from 'lucide-react';
...
export default function SolucoesPage() {
  const { solutions, solutionsError, addSolution, removeSolution, removingSolutionId } =
    useDashboardContext();
    ...
    activeTab === 'all'
      ...
      : solutions.filter((s) =>
          s.category
            .toLowerCase()
            .includes(
              activeTab === 'solar' ? 'solar' : activeTab === 'battery' ? 'bateria' : 'mobilidade'
            )
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 p-5 flex items-center gap-4">
        ...
        <div>
          <p className="text-sm font-semibold text-green-900">Suas soluções impactam o futuro!</p>
                    ...
                    <button
                      onClick={() => {
                        if (window.confirm('Remover esta solução?'))
                          void removeSolution(solution.id);
                      }}
                      disabled={removingSolutionId === solution.id}
          ...
          <div className="rounded-xl border border-green-200 bg-green-50/50 p-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-slate-400">Indisponível</span>
              </div>
              <div className="flex items-center justify-between">
