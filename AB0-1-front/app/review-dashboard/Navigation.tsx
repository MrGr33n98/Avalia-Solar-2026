...
export const sidebarSections: Array<{ title: string; items: DashboardNavItem[] }> = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Respostas das Empresas', href: '/review-dashboard#company-replies', icon: MessageCircle, replies: true },
    ],
  },
  {
    title: 'Oportunidades',
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Bateria e Armazenamento', href: '/review-dashboard#battery', icon: BatteryCharging },
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Notificações', href: '/review-dashboard#notifications', icon: Bell, notifications: true },
    ],
  },
  {
    title: 'Comunidade',
...
export function Header({
  ...
  return (
    <TooltipProvider>
      <header className="flex min-h-[72px] items-center justify-between gap-4 p-4 md:px-6 lg:px-8 border-b border-slate-200 bg-white">
...
export function DesktopSidebar({ repliesCount, notificationsCount }: { repliesCount: number; notificationsCount: number }) {
  return (
            ...
            <div key={section.title} className="space-y-2">
              ...
              <div className="space-y-1">
                {section.items.map((item) => {
                  ...
                  const active = pathname === item.href || (pathname === '/review-dashboard' && item.href === '/review-dashboard');
                  return (
                    <Link
                      key={item.label}
                      href={item.href}
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white/95 px-2 pb-[max(0.5rem,var(--safe-area-inset-bottom))] pt-2 shadow-[0_-10px_28px_-18px_rgba(15,23,42,0.35)] backdrop-blur-xl lg:hidden">
      <div className="mx-auto grid max-w-md grid-cols-5">
        {bottomNav.map((item) => {
          ...
          return (
            <Link key={item.label} href={item.href} className={cn("relative flex min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-1.5 text-[11px] font-bold", active ? "text-emerald-700" : "text-slate-600 hover:text-emerald-700")}>
